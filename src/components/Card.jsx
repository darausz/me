...
export default function Card({ project, card }) {
  ...
  return (
    hover ?
      <div className={`project ${card} ${hover ? "front flipped" : null} ${project.back == null ? "" : ""}`} style={{ filter: "brightness(100%)" }}>
        <div>
          <div className="project-name">{project.name}</div>
          <img className="preview object-cover" src={project.img}></img>
        </div>
        <div className="project-description text-black">
          {project.desc.split("<br/>").join("\n")}
          {project.additional ? <div className="project-description-additional"><br />{project.additional}</div> : null}
        </div>
      ...
      :
      <div className={`project ${floatIn} ${card} ${cardBack} `} style={{"pointer-events": `${disabled ? "none" : "auto"}`}} onMouseEnter={() => { swap() }}>
